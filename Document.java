...
public class Document{
  ...
  */
  ...
  /** Locate the position of the next word beginning at position start
      or later. Return -1 if no words exist after the given start position.
  */
  ...
  */
  ...
  /** Return the next word in the document starting with the first
      word. Ignores punctuation and numbers.
  */
  public String nextWord() throws SpellingException {
    if(this.endWordPos == -1){
      this.endWordPos = 0;
    }
    int begin = startOfNextWord(this.endWordPos);
    if(begin==-1){
      throw new SpellingException("No words remain in the document");
    }
  ...
  /** Return true if the document contains any more words so that a
      ...
      remain in the document.  Punctuation and numbers do not count as words.
  */
